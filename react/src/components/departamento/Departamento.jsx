import { Link } from "react-router-dom";
import { Edit, Trash2 } from "lucide-react";
import React, { useState } from "react";
import AddFuncionarioCategoryOwning from "../modals/AddFuncionarioCategoryOwning";
import axiosClient from "../../axios-client";

export default function Deparatamento({ ID, Nome, responsavel, refetchList }) {
    const [opemModal, setOpenModal] = useState(false);
    const [isActive, setIsActive] = useState(false);

    const onDelete = () => {};

    const toggleActive = () => {
        setIsActive(!isActive);
    };

    const handleAddDepartmentOwner = async (funcionarioID) => {
        // chama api para adicionar responsavel do departmento
        await axiosClient
            .post(`/departamento/assign-user`, {
                funcionario_id: funcionarioID,
                departamento_id: ID,
            })
            .then((data) => {
                alert(data.data.message);
                setOpenModal(false);
                refetchList();
            })
            .catch((err) => {
                alert(err);
            });
        // e atualiza responsavel com o novo responsavel
        // seta modal fechado e atualiza state do departamento
    };

    return (
        <React.Fragment>
            <div className="w-[80%] overflow-hidden sm:w-1/4   rounded-lg bg-white dark:bg-gray-700 m-2 shadow-xl relative">
                <div className="w-full h-1 bg-violet-300 dark:bg-gray-200 absolute top-0"></div>
                <div className="p-4">
                    <Link to={`/departamento/${ID}`}>
                        <h4 className="text-[15px] capitalize pb-4 text-gray-700 dark:text-gray-200">
                            {Nome}
                        </h4>
                    </Link>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Gerencie os funcionários deste departamento com apenas
                        clicando.
                    </p>
                </div>

                {responsavel ? (
                    <Link to={`/funcionario/${responsavel.id}`}>
                        <div className="p-4 flex gap-1 items-center">
                            <img
                                className="w-7 h-7 rounded-full"
                                src="data:image/jpeg;base64,"
                                alt=""
                            />
                            <span className="text-sm text-gray-500 dark:text-gray-400 hover:underline">
                                {responsavel.nome}
                            </span>
                        </div>
                    </Link>
                ) : (
                    <div className="p-4 flex gap-1 items-center justify-between">
                        <span className="text-sm text-gray-500 dark:text-gray-400 hover:underline">
                            sem responsavel
                        </span>
                        <button onClick={(event) => setOpenModal(true)}>
                            <span className="bg-violet-700 p-1 text-sm text-gray-200 dark:text-gray-400 hover:underline">
                                Adicionar
                            </span>
                        </button>
                    </div>
                )}

                <Link
                    to={`/departamentos/atualizar/${ID}`}
                    className="w-4 h-4 cursor-pointer absolute top-4 pb-4 right-2"
                >
                    <Edit className="text-slate-400 hover:text-orange-600 w-[15px] " />
                </Link>
                <button
                    className="w-4 h-4 cursor-pointer absolute top-4 pb-4 right-5 mr-2"
                    onClick={onDelete}
                >
                    <Trash2 className="text-slate-400 hover:text-red-600 w-[15px]" />
                </button>
            </div>
            <AddFuncionarioCategoryOwning
                isOpen={opemModal}
                onAdicionar={(id) => {
                    handleAddDepartmentOwner(id);
                }}
                onClose={() => setOpenModal(false)}
                isActive={isActive}
                toggleActive={toggleActive}
            />
        </React.Fragment>
    );
}
